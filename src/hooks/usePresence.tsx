...
export const usePresence = () => {
  ...
  useEffect(() => {
    ...
    // Poll for online users every 10 seconds as a fallback
  ...
  const handlePresenceUpdate = (presenceMessage: PresenceMessage) => {
    ...
    } else if (presenceMessage.type === 'USER_OFFLINE' && presenceMessage.user) {
      setOnlineUsers((prev) => {
        const newSet = new Set(prev);
        newSet.delete(presenceMessage.user!);
        return newSet;
      });
    } else if (presenceMessage.type === 'ONLINE_USERS_LIST' && presenceMessage.users) {
  ...
  const checkUserOnlineStatus = async (targetUsername: string): Promise<boolean> => {
    try {
      const response = await fetch(`http://localhost:8080/api/users/${targetUsername}/online`);
      if (response.ok) {
